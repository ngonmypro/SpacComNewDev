<?php include "../../../connect/connect_mysql.php";
 ...
 <table id="imgcputb" class="display" cellspacing="0" width="100%"> <!--display nowrap-->
     ...
     <?php $rowint = 1;
      $sql_imgcpu = " SELECT * FROM tblimgcpu WHERE imgcpu_IDdetailcpu = '".$_POST['CPUId']."'";
     ...
     ?>
         <tr style="cursor:pointer;">
             ...
             <td style="font-size:16px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="upload/imagesCPU/<?php echo $row_imgcpu['imgcpu_Nameimg']; ?>" width="100px" height="100px" alt=""> </td>
             <td style="text-align:center;">
               <?php if ($row_imgcpu['imgcpu_Statusmain'] == 0) { ?>
                 <button type="button" class="btn btn-primary btn-xs" title="Set Main" onClick="javascript:MainImgCPU('<?php echo $row_imgcpu['imgcpu_ID'];?>', '<?php echo $_POST['CPUId']; ?>', 'MainImg');"><i class="glyphicon glyphicon-ok"></i></button>
                   <?php if ($row_imgcpu['imgcpu_Status'] == 1) { ?>
                     <button type="button" class="btn btn-success btn-xs" title="Hide" onClick="javascript:HideImgCPU('<?php echo $row_imgcpu['imgcpu_ID'];?>', '<?php echo $_POST['CPUId']; ?>', 'HideImg');"><i class="glyphicon glyphicon-eye-open"></i></button>
                   <?php }else{ ?>
                     <button type="button" class="btn btn-dark btn-xs" title="Show" onClick="javascript:ShowImgCPU('<?php echo $row_imgcpu['imgcpu_ID'];?>', '<?php echo $_POST['CPUId']; ?>', 'ShowImg');"><i class="glyphicon glyphicon-eye-close"></i></button>
                   <?php } ?>
                   <button type="button" class="btn btn-danger btn-xs" onClick="javascript:DelImgCPU('<?php echo $row_imgcpu['imgcpu_ID'];?>', '<?php echo $_POST['CPUId']; ?>','DelImg');"><i class="glyphicon glyphicon-trash"></i></button>
               <?php }else{ ?>
