...
// if everything is ok, try to upload file
    ...
    echo basename( $_FILES["fileToUpload"]["name"]);
		rename ($target_dir . basename( $_FILES["fileToUpload"]["name"]), $target_dir . 'HDD-' . $date . '-' . $_GET['HDDId'] . '.' . $imageFileType);

		$target_file_n = 'HDD-' . $date . '-' . $_GET['HDDId'] . '.' . $imageFileType;
    //echo '</br>'.$target_file_n;
		//--- update data in table employee_tb
		$sql_se = " SELECT imghdd_ID , imghdd_IDdetailhdd FROM tblimghdd WHERE imghdd_IDdetailhdd = '".$_GET['HDDId']."'";
		$result_se = mysql_query($sql_se) or die(mysql_error());
		while ($row_se = mysql_fetch_array($result_se)) {
			$sql_up = " UPDATE  tblimghdd SET  imghdd_Statusmain = '0' WHERE  imghdd_ID = '".$row_se['imghdd_ID']."'";
			$result_up = mysql_query($sql_up) or die(mysql_error());
		}

		$sql = " INSERT INTO  tblimghdd (imghdd_IDdetailhdd, imghdd_Nameimg, imghdd_CreateBy, imghdd_CreateTime)
		VALUES ('".$_GET['HDDId']."', '{$target_file_n}', '{$name}', NOW()) ";
