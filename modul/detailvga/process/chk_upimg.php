...
// if everything is ok, try to upload file
    ...
    echo basename( $_FILES["fileToUpload"]["name"]);
		rename ($target_dir . basename( $_FILES["fileToUpload"]["name"]), $target_dir . 'VGA-' . $date . '-' . $_GET['VGAId'] . '.' . $imageFileType);

		$target_file_n = 'VGA-' . $date . '-' . $_GET['VGAId'] . '.' . $imageFileType;
    //echo '</br>'.$target_file_n;
		//--- update data in table employee_tb
		$sql_se = " SELECT imgvga_ID , imgvga_IDdetailvga FROM tblimgvga WHERE imgvga_IDdetailvga = '".$_GET['VGAId']."'";
		$result_se = mysql_query($sql_se) or die(mysql_error());
		while ($row_se = mysql_fetch_array($result_se)) {
			$sql_up = " UPDATE  tblimgvga SET  imgvga_Statusmain = '0' WHERE  imgvga_ID = '".$row_se['imgvga_ID']."'";
			$result_up = mysql_query($sql_up) or die(mysql_error());
		}

		$sql = " INSERT INTO  tblimgvga (imgvga_IDdetailvga, imgvga_Nameimg, imgvga_CreateBy, imgvga_CreateTime)
		VALUES ('".$_GET['VGAId']."', '{$target_file_n}', '{$name}', NOW()) ";
