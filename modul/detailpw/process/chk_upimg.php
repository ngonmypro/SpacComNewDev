...
// if everything is ok, try to upload file
    ...
    echo basename( $_FILES["fileToUpload"]["name"]);
		rename ($target_dir . basename( $_FILES["fileToUpload"]["name"]), $target_dir . 'PW-' . $date . '-' . $_GET['PWId'] . '.' . $imageFileType);

		$target_file_n = 'PW-' . $date . '-' . $_GET['PWId'] . '.' . $imageFileType;
    //echo '</br>'.$target_file_n;
		//--- update data in table employee_tb
		$sql_se = " SELECT imgpw_ID , imgpw_IDdetailpw FROM tblimgpw WHERE imgpw_IDdetailpw = '".$_GET['PWId']."'";
		$result_se = mysql_query($sql_se) or die(mysql_error());
		while ($row_se = mysql_fetch_array($result_se)) {
			$sql_up = " UPDATE  tblimgpw SET  imgpw_Statusmain = '0' WHERE  imgpw_ID = '".$row_se['imgpw_ID']."'";
			$result_up = mysql_query($sql_up) or die(mysql_error());
		}

		$sql = " INSERT INTO  tblimgpw (imgpw_IDdetailpw, imgpw_Nameimg, imgpw_CreateBy, imgpw_CreateTime)
		VALUES ('".$_GET['PWId']."', '{$target_file_n}', '{$name}', NOW()) ";
