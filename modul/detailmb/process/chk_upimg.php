...
// if everything is ok, try to upload file
    ...
    echo basename( $_FILES["fileToUpload"]["name"]);
		rename ($target_dir . basename( $_FILES["fileToUpload"]["name"]), $target_dir . 'MB-' . $date . '-' . $_GET['MBId'] . '.' . $imageFileType);

		$target_file_n = 'MB-' . $date . '-' . $_GET['MBId'] . '.' . $imageFileType;
    //echo '</br>'.$target_file_n;
		//--- update data in table employee_tb
		$sql_se = " SELECT imgmb_ID , imgmb_IDdetailmb FROM tblimgmb WHERE imgmb_IDdetailmb = '".$_GET['MBId']."'";
		$result_se = mysql_query($sql_se) or die(mysql_error());
		while ($row_se = mysql_fetch_array($result_se)) {
			$sql_up = " UPDATE  tblimgmb SET  imgmb_Statusmain = '0' WHERE  imgmb_ID = '".$row_se['imgmb_ID']."'";
			$result_up = mysql_query($sql_up) or die(mysql_error());
		}

		$sql = " INSERT INTO  tblimgmb (imgmb_IDdetailmb, imgmb_Nameimg, imgmb_CreateBy, imgmb_CreateTime)
		VALUES ('".$_GET['MBId']."', '{$target_file_n}', '{$name}', NOW()) ";
